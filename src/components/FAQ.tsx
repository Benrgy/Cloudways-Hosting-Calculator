          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="bg-white rounded-lg px-6 border border-gray-200 hover:border-gray-300 transition-colors"
              >
        ...
        <div className="text-center mt-16">
          <div className="bg-white rounded-2xl p-8 max-w-2xl mx-auto border border-gray-200">
            <h3 className="text-xl font-bold text-gray-900 mb-4">
              {t('faq.help.title') || 'Still Have Questions?'}
            </h3>
            <p className="text-gray-600 mb-6">
              {t('faq.help.description') || 'Can\'t find the answer you\'re looking for? Contact Cloudways support for personalized assistance.'}
            </p>
            <button
              onClick={() => window.open('https://support.cloudways.com', '_blank')}
              ...
              aria-label="Contact Cloudways support"
            >
              {t('faq.help.button') || 'Contact Support'}
            </button>
          </div>
        </div>
      </div>
